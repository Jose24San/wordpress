...
class spdm_sub_projects_new{
	...
	function getBreadCrumbRecursor($pid){
			...
			$r = $wpdb->get_results("SELECT *  FROM ".$wpdb->prefix."sp_cu_project   where id = '".$wpdb->escape($pid)."' ", ARRAY_A);	
	...
	function makeInput($extra_js){
		...
		<div id="sp_cdm_breadcrumbs"><span><a href="javascript:sp_cdm_load_project(0);"   class="sp_cdm_home"">Home</a> &raquo;</span> <span id="sp_cdm_breadcrumbs_subs">  </span>  </div>';
...
function sub_project_form($form){
	...
	$uid = $current_user->ID;
			...
			$html = '<div class="remodal" data-remodal-id="folder">
...
function parents(){
	...
	global $wpdb,$current_user;
			if($_GET['id'] != '' && $_GET['page'] == 'sp-client-document-manager-fileview'){
	$uid = $_GET['id'];	
}else{
	$uid = $current_user->ID;
}
...
$options = $this->parents();
	...
	$requird = ' required';	
	  ...
	  $html .= '<select name="pid" class="pid_select'.$requird.'"><option value="">None</option>'. $this->recursor( $options,0,0,$chosen).'</select>';
...
$options = spdm_sub_projects_new::parents();
	  ...
	  $html .= '<option value="">None</option>'. spdm_sub_projects_new::recursor( $options).'';
