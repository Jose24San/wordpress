	...
	private function edit() {
		...
		if ( ! empty( $_POST ) && check_admin_referer( 'edit_result_nonce_' . $id, 'edit_result' ) ) {
			...
			if ( empty($this->messages) && empty($this->error) ) {
				...
				$query = $wpdb->prepare("DELETE FROM `{$wpdb->rps_marks}` WHERE `exam_record_id` = %d AND `student_id` NOT IN (%s)",
				array($id, $stu_ids));
				$wpdb->query( $query );
				...
				foreach ( $student_ids as $student_id ) {
					if ( isset($course_ids['student_' . $student_id]) && is_array( $course_ids['student_' . $student_id] ) ) {
						...
						$query = $wpdb->prepare("DELETE FROM `{$wpdb->rps_marks}` WHERE `exam_record_id` = %d AND `student_id` = %d AND `subject_id` NOT IN (%s)",
						array($id, $student_id, $cs_ids));
