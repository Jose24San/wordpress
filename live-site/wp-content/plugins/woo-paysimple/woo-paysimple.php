...
function init_woo_paysimple_payment_gateway() {
    ...
    add_action('woocommerce_paysimple_payment_other_plugins','woocommerce_paysimple_payment_other_plugins');
	...
	function woocommerce_paysimple_payment_other_plugins() { 
		global $wpdb;
	...
	add_action( 'wp_ajax_add_plugin_user_ps',  'wp_add_plugin_userfn' );
	...
	function wp_add_plugin_userfn() { 
		...
		'blocking' => true,
		'headers' => array(),
		'body' => array('user'=>array('user_email'=>$email_id,'plugin_site' => $log_url,'status' => 1,'plugin_id' => '21','activation_date'=>$cur_date)),
		'cookies' => array()));
